...
@st.dialog("⭐️ LEVEL SETTINGS", width="large")
def edit_mission_dialog(item_id):
    # ดึงข้อมูลจากฐานข้อมูลมาเป็นค่าตั้งต้นครั้งแรกครั้งเดียว
    res = supabase.table("cleaning_plans").select("*").eq("id", item_id).execute()
    if not res.data: 
        st.error("ไม่พบข้อมูลด่านนี้"); return
    item = res.data[0]

    # --- 🆕 ใช้ Session State พักข้อมูล (ตะกร้าสินค้า) ---
    if f"temp_acts_{item_id}" not in st.session_state:
        st.session_state[f"temp_acts_{item_id}"] = item.get('activities', [])
    if f"temp_tools_{item_id}" not in st.session_state:
    ...
    with t1:
        u_name = st.text_input("ชื่อด่าน", value=item['zone_name'])
        u_staff = st.text_input("ฮีโร่ผู้รับผิดชอบหลัก", value=item.get('responsible_staff', ''))
        st.info("💡 ปรับแต่งกิจกรรมและอุปกรณ์ใน Tab อื่นๆ แล้วกดปุ่มบันทึกด้านล่างทีเดียว")
    ...
    with t2:
        ...
        with st.container(border=True):
            ...
            if st.button("➕ เพิ่มลงตะกร้ากิจกรรม", use_container_width=True):
                final_act = custom_act if act_sel == "➕ พิมพ์เพิ่มเอง..." else act_sel
                if final_act:
                    st.session_state[f"temp_acts_{item_id}"].append({
                        "name": final_act, "people": int(num_people), "hours": int(hrs)
                    })
                    st.rerun() # rerun ภายใน dialog เพื่อแสดงรายการที่เพิ่มใหม่
        ...
        for i, a in enumerate(st.session_state[f"temp_acts_{item_id}"]):
            ...
            c2.write(f"👥 {a.get('people', 1)} คน")
            c3.write(f"⏱️ {a.get('hours', 0)} ชม.")
            if c4.button("🗑️", key=f"del_act_temp_{i}"):
    ...
    with t3:
        ...
        if c_btn.button("➕ ลงตะกร้า", use_container_width=True):
            st.session_state[f"temp_tools_{item_id}"].append({"item": final_tool, "amount": int(qty)})
            st.rerun()
        ...
        for i, t in enumerate(st.session_state[f"temp_tools_{item_id}"]):
            col_t, col_b = st.columns([5, 1])
            col_t.markdown(f"📦 **{t['item']}** x{t['amount']}")
            if col_b.button("🗑️", key=f"del_t_temp_{i}"):
                st.session_state[f"temp_tools_{item_id}"].pop(i)
                st.rerun()

    # --- 💾 ปุ่มบันทึกไม้ตาย (อยู่ท้ายสุดของทุก Tab) ---
    st.divider()
    if st.button("💾 บันทึกการเปลี่ยนแปลงทั้งหมด", type="primary", use_container_width=True):
        ...
        supabase.table("cleaning_plans").update(update_data).eq("id", item_id).execute()
        
        # ล้างข้อมูลชั่วคราวออก
        del st.session_state[f"temp_acts_{item_id}"]
        del st.session_state[f"temp_tools_{item_id}"]
        
        st.session_state.success_msg = "✅ บันทึก Mission ทั้งหมดสำเร็จแล้ว!"
        st.rerun()

    with t4:
        st.error("⚠️ การลบด่านจะระเบิดข้อมูลทิ้งทั้งหมด")
        if st.button("🧨 ยืนยันการลบด่าน", use_container_width=True):
            supabase.table("cleaning_plans").delete().eq("id", item_id).execute()
            # ล้าง temp ออกถ้ามีการลบ
            if f"temp_acts_{item_id}" in st.session_state: del st.session_state[f"temp_acts_{item_id}"]
            if f"temp_tools_{item_id}" in st.session_state: del st.session_state[f"temp_tools_{item_id}"]
            st.session_state.last_c = None 
            st.session_state.success_msg = "🧨 ระเบิดด่านทิ้งเรียบร้อย!"
            st.rerun()
